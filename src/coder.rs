    ...
    pub(crate) fn pack_array(
        ...
        match coding {
            TNG_COMPRESS_ALGO_BWLZH1 | TNG_COMPRESS_ALGO_BWLZH2 => {
                let mut output =
                    vec![0; 4 + bwlzh_get_buflen(usize::try_from(length).expect("usize from i32"))];
                let i = length;
                let j = length;
                let k = length;
                let n = length;
                ...
                let mut cnt = 0;
                let mut pval: Vec<u32> = vec![0; usize::try_from(n).expect("usize from i32")];

                let mut most_negative = FixT::MAX31BIT as i32;
                for i in 0..n {
                    if input[i as usize] < most_negative {
                        most_negative = input[i as usize];
                    }
                }
                ...
                for i in 0..n_atoms {
                    for j in 0..3 {
                        for k in 0..n_frames {
                            let item = input[(k * 3 * n_atoms + i * 3 + j) as usize];
