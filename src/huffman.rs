...
mod tests {
    ...
    use super::ptngc_comp_conv_to_huffman;
    ...
    fn single_symbol() {
        ...
        let mut huffman = vec![0; 1000];
        let mut huffman_len = 0;
        let mut huffman_dictlen = 0;
        let mut huffman_dict_unpackedlen = 0;
        let mut huffman_dict = vec![0; 131077];
        let mut huffman_dict_unpacked = vec![0; 131077];
        ...
        assert_eq!(huffman_len, 1);
        assert_eq!(huffman_dictlen, 9);
        assert_eq!(huffman_dict_unpackedlen, 46);
        assert_eq!(huffman[..huffman_len], vec![0]);
        assert_eq!(
            huffman_dict[..huffman_dictlen],
            vec![42, 0, 0, 0, 0, 0, 0, 0, 33]
        );
        let mut expected = vec![0; 46];
        expected[0] = 42;
        *expected.last_mut().unwrap() = 1;
        assert_eq!(huffman_dict_unpacked[..huffman_dict_unpackedlen], expected);
    }
    ...
    fn two_symbols() {
        ...
        assert_eq!(huffman_dict_unpackedlen, 24);
        assert_eq!(huffman[..huffman_len], vec![80]);
        assert_eq!(
            huffman_dict[..huffman_dictlen],
            vec![20, 0, 0, 0, 33, 0, 66]
        );
    ...
    fn large_values() {
        ...
        assert_eq!(huffman_len, 1);
        assert_eq!(huffman_dictlen, 130);
        assert_eq!(huffman_dict_unpackedlen, 1005);
        assert_eq!(huffman[..huffman_len], vec![80]);
        let mut expected = vec![0; huffman_dictlen];
        expected[0] = 233;
        expected[1] = 3;
        expected[huffman_dictlen - 1] = 16;
        expected[huffman_dictlen - 2] = 134;
        assert_eq!(huffman_dict[..huffman_dictlen], expected);

        let mut expected = vec![0; huffman_dict_unpackedlen];
        expected[0] = 233;
        expected[1] = 3;
        expected[huffman_dict_unpackedlen - 1] = 1;
        expected[huffman_dict_unpackedlen - 2] = 1;
        assert_eq!(huffman_dict_unpacked[..huffman_dict_unpackedlen], expected);
    }
    ...
    fn empty() {
        ...
        assert_eq!(huffman_len, 0);
        assert_eq!(huffman_dictlen, 0);
        assert_eq!(huffman_dict_unpackedlen, 0);
        assert_eq!(huffman[..huffman_len], vec![]);
        assert_eq!(huffman_dict[..huffman_dictlen], vec![]);
        assert_eq!(huffman_dict_unpacked[..huffman_dict_unpackedlen], vec![]);
    }
}
